...
function err = genetic_attempt(z)
    %for i=0:5:90
        %figure
    load('ref','ref')
    ref
    ...
    l=12
    k=12
    [x,y]= draw_rectangle([8,8],k,l,z(1),'black')
    z(1)
    j=1
    A(j,:) = x
    B(j,:)= y;
    ...
    err(1) = sum(abs(tempA-tempB))/minlength
    err(1)
    hold off 
    drawnow
    end
...
if flag==1
vertices = [ x1 y1 ; (x2+(1*rand(1))) (y2+(1*rand(1))) ;...
    x2 y2 ; (x3+(1*rand(1))) (y3+(1*rand(1))) ;...
    x3 y3 ; (x4+(1*rand(1))) (y4+(1*rand(1)))  ;...
    x4 y4  ; (x1+(1*rand(1))) (y1+(1*rand(1))) ] ;
else
...
injInd = delTri.pointLocation(1, 1);
prodInd = delTri.pointLocation(17, 17);
