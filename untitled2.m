...
global ii
    ...
    k=12
    l=12
    [x,y]= draw_rectangle([10,10],k,l,45,'black')
    kk=0
    ll=0
    ii=0
    A(j,:) = x
    B(j,:)= y;
    A(j,1)
    ref = attempt(A(j,1),A(j,2),A(j,3),A(j,4),B(j,1),B(j,2),B(j,3),B(j,4),1);
    hold off
    j=0
    kk=k
    ll=l
   ...
   %     ll=l
    for i=0:90:90
        ii=i
        figure
    [x,y]= draw_rectangle([10,10],k,l,i,'black')
    j=j+1
    A(j,:) = x
    B(j,:)= y;
    temp = attempt(A(j,1),A(j,2),A(j,3),A(j,4),B(j,1),B(j,2),B(j,3),B(j,4),0);
    minlength = min(length(temp),length(ref))
    tempA = temp(1:minlength,1)
    tempB= ref(1:minlength,1)
    err(j,4) = sum(abs(tempA-tempB))/minlength
    err(j,3) = i
    err(j,2) = l
    err(j,1) = k
    err(j,:)
    ...
    save('err','err')
    FileData = load('err.mat')
    csvwrite('error.csv',FileData.err)
    
%end

function[x_coor,y_coor]= draw_rectangle(center_location,L,H,deg,rgb)
global kk
global ll
global ii
theta=deg*pi/180;
...
function[wellConcentration]=attempt(x1, x2,x3,x4,y1,y2,y3,y4,flag)
global kk
global ll
global ii
N     = 24;
...
if flag==1
vertices = [ x1 y1 ; (x2+(0.5*rand(1))) (y2+(0.5*rand(1))) ;...
    x2 y2 ; (x3+(0.5*rand(1))) (y3+(0.5*rand(1))) ;...
    x3 y3 ; (x4+(0.5*rand(1))) (y4+(0.5*rand(1)))  ;...
    x4 y4  ; (x1+(0.5*rand(1))) (y1+(0.5*rand(1))) ] ;
else
...
for iter = unique(tags)'
    ...
    aperture(hit) = 10^-3 * 0.5; % This choice is random in more than one way..
end
...
fname = sprintf('mrst-2020b/project_images/dfm_%d_%d_%d',kk,ll,ii);
saveas(gcf,fname)
axis equal, axis off
...
injInd = delTri.pointLocation(3, 3);
prodInd = delTri.pointLocation(18, 18);
...
fname = sprintf('mrst-2020b/project_images/final_simulation_%d_%d_%d',kk,ll,ii);
saveas(gcf,fname)
...
fname = sprintf('mrst-2020b/project_images/water_cut_%d_%d_%d',kk,ll,ii);
saveas(gcf,fname)
end
